...
export class DetalleComandasController {
 ...
 create(@Body() createDetalleComandaDto: CreateDetalleComandaDto) {
  ...
  findAll(@Request() req) {
    ...
    return this.detalleComandasService.findAll(restauranteId);
  }
  ...
  @ApiResponse({ status: 401, description: 'No autenticado.' })
  @ApiResponse({ status: 403, description: 'No autorizado (rol incorrecto).' })
  async getComandasForCocinero(@Request() req): Promise<Comanda[]> {
    this.logger.log(`Usuario (Cocinero/Admin) solicitando comandas pendientes para cocina.`);
    const restauranteId = req.user.id_restaurante;
    return this.detalleComandasService.findComandasForCocineroDashboard(restauranteId);
  }
  ...
  async getComandasForMesonero(@Request() req): Promise<Comanda[]> {
    this.logger.log(`Usuario (Mesonero) solicitando comandas activas.`);
    const restauranteId = req.user.id_restaurante;
    return this.detalleComandasService.findComandasForMesoneroDashboard(restauranteId);
  }
